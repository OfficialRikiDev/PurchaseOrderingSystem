...
<body class="overflow-hidden">
    <div class="flex bg-gray-700 h-screen" x-data="{ isSidebarExpanded: false }">
        ...
        <div class="flex-1 flex flex-col w-full">
            ...
            <main class="flex flex-col p-6 text-white w-full gap-4 h-full">
                <div class="stats stats-vertical lg:stats-horizontal shadow w-full h-1/5">
                    <div class="stat ">
                ...
                <div class="flex flex-row gap-2 w-full">
                    <span class="font-bold text-lg">Budget Activity Board  </span>
                    <button id="setBudgetBtn" class="btn btn-xs btn-success place-self-end">Set Budget</button>
                </div>
                <div class="overflow-x-auto rounded h-full bg-slate-600">
                    <table class="table table-xs table-pin-rows  p-3 ">
                        ...
                        <tbody>
                            <?php
                                $list = array_reverse($budget->getBudget());
                                if($list){
                                    foreach($list as $b){
                                        echo '<tr>
                                        <th></th>
                                        <td>'.$data->getAccountById($b['set_by'])['username'].'</td>
                                        <td>₱'.number_format($b['allocation'], 2).'</td>
                                        <td>'.date("M d, Y", strtotime($b['date'])).'</td>
                                        <td>'.(!$b['prev_alloc'] ? '-' : ($b['allocation'] >= $b['prev_alloc'] ? '<i class="fas fa-level-up-alt text-success"></i>': '<i class="fas fa-level-down-alt text-error"></i>')).'</td>
                                        <th></th>
                                    </tr>';
                                    }
                                }
