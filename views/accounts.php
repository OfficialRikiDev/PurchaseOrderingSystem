...
<div class="overflow-x-auto w-100">
    <label for="addItemModal" class="btn btn-xs btn-primary mb-2 float-right" onclick="addAccountModal.showModal()">Add Account</label>
...
<dialog id="addAccountModal" class="modal">
    <div class="modal-box">
    <form action="/backend/action.php" method="post" id="addAccountForm">
        <h3 class="font-bold text-lg pb-3">Add Account</h3>
        <hr>
        <div class="form-control w-full">
        <label class="label">
            <span class="label-text">Account Email</span>
        </label>
        <input type="email" name="accountEmail" placeholder="Enter account email" class="input input-bordered w-full max-w-full" />
        <label class="label">
            <span class="label-text">Account Username</span>
        </label>
        <input type="text" name="accountUser" placeholder="Enter account username" class="input input-bordered w-full max-w-full" />
        <label class="label">
            <span class="label-text">Account Password</span>
        </label>
        <input type="password" name="accountPassword" placeholder="Enter password" class="input input-bordered w-full max-w-full" />
        <label class="label">
            <span class="label-text">Account Role</span>
        </label>
        <select name="accountRole" class="select select-bordered">
            <option value="0">Super Admin</option>
            <option value="1">Admin / Chairman</option>
            <option value="2">Supplier</option>
            <option value="3">Property Custodian</option>
        </select>
        </div>
        <div class="modal-action">
            <input type="hidden" name="addAccount">
            
        </div>
        </form>
        <form method="dialog">
            <!-- if there is a button in form, it will close the modal -->
            <button class="px-5 btn rounded-md bg-sky-600 text-white" form="addAccountForm" type="submit">Add</button>
